...
\textbf{(i) Auto (Dask default):} Uses \dask's default configuration with the standard 128~MB chunk size target.
This represents the out-of-the-box behavior that users experience without manual tuning.
While \dask allows manual chunk size adjustment, such tuning requires trial-and-error across different operators and volumes, which is precisely the problem memory-aware chunking addresses through automated prediction.

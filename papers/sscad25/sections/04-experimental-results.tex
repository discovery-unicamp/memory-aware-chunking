...
Figure~\ref{fig:performance-scaling} illustrates this reliability advantage using a $200\times200\times200$ volume: while auto and evenly-split chunking achieve 35\% faster execution times with 1-4 workers (e.g., 42s vs 65s with 4 workers), they encounter \ac{OOM} failures beyond 4 workers.
Memory-aware chunking shows consistent scaling from 1 to 8 workers, with execution time decreasing from 120s to 72s—a 40\% reduction that demonstrates effective parallelization despite conservative chunk sizing.
The apparent ``overhead'' compared to failed methods is actually the cost of successful completion versus job failure.

...
We processed synthetic seismic volumes with dimensions ranging from $100^3$ (3.8~MB) to $400^3$ (244~MB), representing typical sub-volumes in production workflows.
We tested all combinations of dimensions using values 100, 200, 300, and 400 for each axis (inlines × xlines × samples), creating 64 distinct volume configurations.
...
Each configuration was executed 5 times, with memory usage sampled at 100~ms intervals using the process-level \ac{RSS} metric.
...
\EB{Daniel, esta tabela não bate com o gráfico da figura 2. No caso de 1 a 4 nós, onde todos funcionam, dá pra ver que o auto e o evently\_split fica em torno de 15 segundos, no entanto, o memaware também fica em torno deste tempo, em vez de 28s. Poderia verificar isso?}
...
\begin{table}[htb]
    \centering
    \caption{Performance comparison for successful runs (median values) \EB{Aqui tem os tempos apenas para as configurações/runs que funcionaram em todas as abordagens?}}
...
Memory-aware chunking halved peak memory usage relative to competitors, reducing median consumption from 2.62~GB to 1.24~GB.
\EBC{This reduction comes at the cost of increased execution time, with memory-aware taking 79\% longer than the baseline approaches.}{Revisitar esta frase depois de avaliar o comentário anterior.}
...
Varying the \EBC{safety factor}{Acho que faltou explicar o que é este safety factor.} between 0.6 and 1.0 established that the default 0.8 balances speed and safety (Table~\ref{tab:safety-factor}).

...
Table~\ref{tab:oom-analysis} shows that memory-aware chunking prevented every OOM failure across all 768 trials, while auto chunking experienced 243 OOM failures and evenly-split experienced 243 OOM failures.
...
Memory-aware chunking achieved a 52\% reduction in peak memory usage relative to competitors, reducing median consumption from 2.44~GB to 1.16~GB.
...
\textcolor{blue}{The performance degradation can be attributed to the memory-aware algorithm's tendency to create chunks with more balanced dimensions, approaching cubic shapes when possible. This strategy optimizes memory usage but may lead to suboptimal data locality and increased communication overhead for non-cubic input volumes. For instance, processing a volume with dimensions $100 \times 100 \times 400$ results in chunks that deviate from the natural data layout, causing additional data movement and synchronization costs compared to chunks that better align with the elongated dimension.}

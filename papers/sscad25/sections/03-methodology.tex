...
\begin{table}[htb]
    ...
    \begin{tabular}{lccc}
        \toprule
        \textbf{Operator} & \textbf{$R^2$} & \textbf{\ac{RMSE} (\ac{MB})} & \textbf{Training Samples} \\
        \midrule
...
\textbf{(i) Memory Safety Guarantees:} By modeling peak memory usage including all intermediate allocations, the system prevents \ac{OOM} failures that plague production deployments.
...
A memory-intensive operation like \ac{GST3D} receives smaller chunks than a lightweight operation like element-wise multiplication.

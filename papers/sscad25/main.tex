\documentclass[12pt]{article}

\usepackage{assets/template/sbc-template}
\usepackage{graphicx,url}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}

% Additional packages needed for the paper
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{algorithmic}
\usepackage{algorithm}
\usepackage{booktabs}
\usepackage{subcaption}
\usepackage{listings}
\usepackage{xcolor}
\usepackage[printonlyused]{acronym}
\usepackage{multirow}
\usepackage{array}

\lstset{
    basicstyle=\footnotesize\ttfamily,
    breaklines=true,
    frame=single,
    numbers=left,
    numberstyle=\tiny,
    captionpos=b,
    tabsize=2
}

% Define acronyms used in the paper
\acrodef{HPC}{High-Performance Computing}
\acrodef{OOM}{Out-Of-Memory}
\acrodef{CPU}{Central Processing Unit}
\acrodef{GPU}{Graphics Processing Unit}
\acrodef{RAM}{Random Access Memory}
\acrodef{API}{Application Programming Interface}
\acrodef{I/O}{Input/Output}
\acrodef{OS}{Operating System}
\acrodef{RSS}{Resident Set Size}
\acrodef{PSS}{Proportional Set Size}
\acrodef{USS}{Unique Set Size}
\acrodef{GST3D}{Gradient Structure Tensor 3D}
\acrodef{RMSE}{Root Mean Square Error}
\acrodef{GB}{Gigabyte}
\acrodef{MB}{Megabyte}
\acrodef{TB}{Terabyte}
\acrodef{LSTM}{Long Short-Term Memory}
\acrodef{VM}{Virtual Machine}
\acrodef{HDFS}{Hadoop Distributed File System}
\acrodef{PVFS}{Parallel Virtual File System}
\acrodef{HDF5}{Hierarchical Data Format version 5}

\sloppy

\title{Preventing Out-Of-Memory Errors in Dask through Automated Memory-Aware Chunking}

\author{Daniel De Lucca Fonseca\inst{1}, Carlos Alberto Astudillo Trujillo\inst{1}, Edson Borin\inst{1}}

\address{Institute of Computing -- University of Campinas (UNICAMP)\\
  Av. Albert Einstein, 1251 -- 13083-852 -- Campinas -- SP -- Brazil
  \email{d182873@dac.unicamp.br, castudillo@unicamp.br, borin@unicamp.br}
}

\input{macros}

\begin{document}

\maketitle

\begin{abstract}
\input{sections/00-abstract}
\end{abstract}

\input{sections/01-introduction}
\input{sections/02-related-work}
\input{sections/03-methodology}
\input{sections/04-experimental-results}
\input{sections/05-conclusion}


%---------------------------------------------------------------------------------------
\section*{Acknowledgments}
\label{sec:ack}
%---------------------------------------------------------------------------------------
The authors would like to thank PETROBRAS for funding this study. Prof. Borin also received funding from CNPq (315399/2023-6) and Fapesp (2013/08293-7).

\bibliographystyle{assets/template/sbc}
\bibliography{bibliography}

\end{document}
...
Accurate memory measurement in Python applications requires a systematic approach to address challenges identified in Section~\ref{sec:mmc-common-challenges-and-pitfalls} and elaborated further in Appendix~\ref{ch:appendix-challenges-pitfalls}.
This section outlines comprehensive best practices for minimizing measurement biases, ensuring reproducible results, and reliably determining an application's actual memory footprint.
...
Docker and similar container technologies offer effective isolation through namespace encapsulation and \ac{cgroup} resource management.
...
\begin{enumerate}
    ...
    \item \textbf{Explicit Resource Management:} Containers allow defined memory constraints using flags such as \texttt{–memory} or \ac{cgroup} configurations, helpful in validating memory usage limits (detailed in Section~\ref{subsec:mmc-bestpractices-validation}).
\end{enumerate}
...
Cross-validation of container metrics against system-level metrics (\eg, via Linux's \texttt{/proc/[pid]/smaps}) is essential to ensure accuracy.
...
This method suits short-lived batch experiments or iterative test scenarios, provided the overhead from subprocess creation remains acceptable.

\paragraph{Preventing Cross-Contamination.}
Effective isolation demands that each experiment commence from a pristine environment.
...
\begin{itemize}
    ...
    \item \textbf{Kernel or Interpreter Reset in Interactive Environments:} Interactive environments like Jupyter notebooks retain memory references across executions.
    Restarting the kernel ensures the complete removal of lingering objects and facilitates accurate baseline memory assessment.
...
Selecting appropriate measurement frequency and granularity is crucial to capture memory usage without excessive overhead accurately:
...
Measuring peak memory usage, such as through Python's \texttt{resource.getrusage} or \texttt{psutil.Process().memory\_info().rss}, ensures capturing the worst-case scenarios.
...
Rigorous validation ensures the reliability and accuracy of memory measurements:
...
Overlaying different data sources, such as \texttt{psutil}, \texttt{resource}, and \texttt{tracemalloc}, allows detection of divergences between Python-level allocations and system-level reported memory, clarifying allocation behaviors and memory usage subtleties.
...
Adherence to these best practices ensures precise, reliable, and reproducible memory profiling, which is essential for efficient resource management in Python-based high-performance computing and large-scale computational contexts.
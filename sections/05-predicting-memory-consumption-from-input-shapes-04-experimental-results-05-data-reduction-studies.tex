...
Figure~\ref{fig:cross_data_reduction_and_residual} shows \ac{RMSE} and residual metrics as the \EBADD{training} dataset shrinks. \EB{Qual a principal conclusão do experimento?}

\EB{O problema desta abordagemé que os experimentos com as maiores amostras são os experimentos mais custosos. Será que é possível derivar bons modelos apenas com amostras pequenas e, talvez, médias?}
...
Figures~\ref{fig:residual_metrics_by_sample_size_operators} and~\ref{fig:metrics_evolution_sample_size_operators} \EBC{analyze performance by operator.}{Daniel, para cada figura, procure oferecer uma breve descrição do que a figura está mostrando, apontar as principais características a serem observadas e as conclusões que podem ser derivadas.}
...
\ac{GST3D} is more sensitive, while Envelope and Gaussian Filter remain robust with fewer \EBRPD{configurations}{data samples}.

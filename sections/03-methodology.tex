...
In our experiments, training on as few as 30 samples yields $R^2>0.999$, as indicated in Table~\ref{tab:feature-importance}.
...
The implementation leverages \dask's plugin architecture \EBADD{as follows}:
...
The predictors are stored as lightweight pickle files (typically \textless 1~KB each) that load in milliseconds, adding negligible overhead to graph construction.

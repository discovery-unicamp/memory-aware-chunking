...
\begin{itemize}
    ...
    \item \emph{Slight Speed Trade-Off:} While memory-aware chunking is typically slower than the fastest alternative (\eg, evenly-split or auto-chunking) on small-to-medium data, it remains fairly close to the best strategy in many scenarios. As dataset sizes increase, the ability to avoid \ac{OOM} errors becomes paramount, offsetting the extra runtime and positioning memaware as a robust fallback in virtually any setting.
...
\begin{itemize}
    ...
    \EB{Uma dúvida que restou é se nossa abordagem de chunking é muito conservadora e acaba gerando chunks muito pequenos. Um possível teste para tentar clarificar este ponto seria rodar com um modo agressivo do memaware onde, em vez de ajustar o chunking com o recomendado pelo memaware, nós utilizaríamos um fator de redução para ajustar a predição de memória e permitir que o algoritmo escolha chunks maiores. Algo como memaware-0.9 ou memaware-0.5 onde a predição de memória seria 10\% (ou 50\% menor do que o predito pelo modelo). Se estas versões agressivas causarem OOM, é um indício de que estamos realizando predições próximas do limiar.}
    \EB{Acho que podemos deixar este experimento para o artigo ou versão final da dissertação -- pós-defesa. Recomendo deixar para trabalhar nisso após fecharmos o texto para a banca.}

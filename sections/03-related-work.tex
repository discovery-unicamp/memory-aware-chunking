...
This classification scheme simplifies integration into batch schedulers (\ac{LSF}~\cite{ibm_spectrum_lsf_2020}, \ac{PBS}, \ac{TORQUE}~\cite{staples2006torque}) by limiting the range of predicted memory requests.
...
By contrast, this thesis's approach focuses on shape-based, per-task prediction within Python frameworks, allowing more granular control over memory usage and chunk sizes.
...
Instead, it highlighted the challenges of container overhead and system-level tracing, which the present thesis addresses through careful container-based measurement of Python memory usage, in contrast to the broader, high-level discussion in the survey.


\section{Practical Prediction for Large Memory Jobs in HPC}
\label{sec:rw-practical-prediction}

Li \etal~\cite{li2019} addressed large-memory \ac{HPC} jobs by splitting the prediction model into two stages.
A classifier first flags whether a job likely belongs to a high-memory category.
The model applies a dedicated regressor if the classifier identifies a high-memory job.
...
The present thesis uses a different strategy by directly modeling the memory footprint from array or tensor shapes, enabling runtime adjustments (\eg, smaller chunk sizes) for data-intensive Python tasks.
...
The feedback loop is conceptually similar to the iterative refinements used here. However, this thesis focuses on achieving accurate, shape-based memory modeling at the operator level (\eg, Gaussian filtering), combined with careful container-based measurement.
...
Their model combined analytical insight with \ac{ML} (Lasso, Ridge, \ac{MLP}) to estimate peak memory usage with around 95--99\% accuracy.
...
However, it remained Spark-specific and did not explicitly account for container overhead or \ac{HPC} schedulers.
...
The current thesis adopts a similar shape-based logic for Python tasks but integrates additional container-oriented measurements and \ac{HPC} constraints.
...
It also does not provide details on the handling of container overhead.
The work demonstrates that \ac{ML} can succeed at scale but does not differentiate sub-job tasks (\eg, Dask-based array computations).
...
They found that features like user ID and application name strongly correlate with memory usage, motivating heavy feature engineering (\eg, one-hot encoding of top apps, user clustering).
...
By contrast, the method in this thesis learns memory usage from the size of input arrays or tensors and applies it to per-chunk tasks. This method is particularly suited to Python data analytics and seismic workloads.
...
The present work adopts a shape-based method in Python settings (\eg, Dask) and incorporates detailed container-aware measurement.
...
\begin{table}[htbp]
    ...
    Container Overhead indicates whether the metrics originate from container or cgroup-level monitoring.
    Evaluate on Dask indicates whether the validation occurred within a Dask environment.
    \label{tab:related-work-compare}
    \begin{tabular}{>{\centering\arraybackslash}m{2.3cm}m{5cm}m{1.2cm}m{1.3cm}m{2.0cm}m{1.6cm}}
        \toprule
        \textbf{Reference}
        & \textbf{Focus}
        & \textbf{\ac{ML}-based}
...
Few studies account for container overhead, and none fully validate on Dask.
...
Some ideas were adapted from Rodrigues \etal~\cite{rodrigues2016} and Tanash \etal~\cite{tanash2021ampro} regarding offline \ac{ML} model building, although the target here is more granular (per-task) and shape-oriented rather than aggregate job logs.
...
Overall, this thesis's approach synthesizes shape-based modeling, container-aware measurement, and a flexible chunking system for Dask, bridging important gaps identified in previous work.
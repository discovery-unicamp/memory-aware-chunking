...
Overestimates waste resources, whereas underestimates risk out-of-memory (\ac{OOM}) errors.
...
Containerization (e.g., Docker~\cite{docker}) further complicates accounting due to caching and shared kernel resources.
...
Section~\ref{sec:mmc-traceq} introduces the \texttt{traceq} library, designed to allow a memory-footprint free profiling for both Python-internal (e.g., \texttt{tracemalloc}) and system-level metrics (e.g., \texttt{psutil}).

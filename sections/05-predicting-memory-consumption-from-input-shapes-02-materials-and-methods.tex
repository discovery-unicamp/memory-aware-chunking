...
This section explains the complete methodology for gathering memory-consumption data, generating shape-based features, training, and evaluation the predictive models.
...
To address these concerns, the methodology includes a validation step using the publicly available F3 real seismic dataset.
The results show that memory-consumption patterns and shape-to-usage relationships derived from synthetic volumes closely match those observed in real seismic data.
...
This phase consists in evaluating the \texttt{.prof} files produced by \traceq, extracting peak memory usage, and associating it with the input shape.
...
After selecting hyperparameters, the script retrains the model on the entire training set and then evaluates the final performance on the held-out test set.
    ...
    \resizebox{\textwidth}{!}{
        \begin{tikzpicture}[
            ...
            \node[box, below=of p4a, text width=3.8cm] (p4b) {
                \textbf{Process}\\
                Final training on entire train set\\
                + Test set evaluation
            };

\chapter{Challenges and Pitfalls in Accurate Memory Measurement}
\label{ch:appendix-challenges-and-pitfalls-in-accurate-memory-measurement}
%
%
...
%    Both Python's memory allocator (\eg, pymalloc~\cite{pymalloc}) and the underlying C libraries can cause memory fragmentation.
...
%    For instance, Python maintains internal caches for frequently used objects (\eg, small integers and strings), while the \ac{OS} uses disk and memory caches to optimize \ac{I/O} operations.
...
%    Scientific applications often rely on third-party libraries (\eg, NumPy~\cite{numpy}, pandas~\cite{pandas}, TensorFlow~\cite{tensorflow}), which may implement their own memory management strategies.
...
%Profiling tools, whether external (\eg, psutil~\cite{psutil}, resource~\cite{importlib_resources}) or internal (\eg, tracemalloc~\cite{tracemalloc}), introduce overhead that can influence the very metrics they aim to capture.
...
%    \item Resetting the environment (\eg, restarting the Python interpreter or Jupyter kernel) before each measurement to ensure a clean slate.
...
%
%\DF{Vale a pena adicionar uma descrição detalhada de como o \ac{GST3D} funciona e como ele foi implementado nos apêndices?}
%

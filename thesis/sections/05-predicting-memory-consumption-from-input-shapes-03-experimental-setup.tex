...
All experiments ran on a single \ac{HPC} node at the \ac{UNICAMP} Discovery Labs, equipped with an Intel\textregistered\ Xeon\textregistered\ Silver 4310 processor, 256~\ac{GB} of \ac{RAM}, and two \ac{RTX}~A6000 \ac{GPU}s.
Despite available \ac{GPU} resources, all memory profiling runs exclusively used \ac{CPU} execution to ensure comparability across input shapes.
...
A single shell script, \texttt{scripts/experiment.sh}\footnote{Experiment Script. Available at \url{https://github.com/discovery-unicamp/memory-aware-chunking/blob/main/experiments/02-predicting-memory-consumption-from-input-shapes/scripts/experiment.sh}. Accessed: 2025-04-29.}, orchestrated the full workflow, including:
\begin{enumerate}
    ...
    \item Memory profiling using \traceq, which captured \ac{RSS} measurements over time.
...
The script looped over all permutations of \texttt{(inlines, xlines, samples)}, parsing \traceq outputs and joining them with shape metadata.
...
This method enabled a controlled exploration of both standard and extreme input configurations.
...
Users can download the dataset from the Terranubis website~\cite{f3dataset}.
The experiments compared memory usage predictions from the trained models against actual measurements on this dataset, enabling evaluation of model generalization to real-world data.
...
Each shape configuration ran in a dedicated Docker container with exclusive \ac{CPU} pinning to reduce runtime variance.
The experiments evaluated three memory-intensive seismic operators: \emph{Envelope}, \emph{\ac{GST3D}}, and a \emph{3D Gaussian Filter}.
The analysis treated these operators as black-box transformations that received seismic volumes and returned transformed data, without internal modifications or visibility into their implementation.

The container startup routine activated \traceq, which monitored the Python process and recorded \ac{RSS} snapshots at regular intervals.
Upon operator completion, \traceq wrote a \texttt{.prof} file containing the memory usage timeline.
...
\begin{itemize}
    ...
    \item \textbf{Shape and Operator Metadata:} Inline, xline, sample dimensions, applied seismic operator, and associated attributes.
...
The entire experiment is publicly available at a Github repository~\footnote{Experiment Folder. Available at \url{https://github.com/discovery-unicamp/memory-aware-chunking/blob/main/experiments/02-predicting-memory-consumption-from-input-shapes}. Accessed: 2025-04-29.}.
...
All artifacts for this experiment reside in the folder \texttt{experiments/02-predicting-memory-consumption-from-input-shapes}.
A detailed \texttt{README.md} file describes environment requirements, including Docker, and explains how to run each step.

The \texttt{scripts/experiment.sh} script~\footnote{Experiment Script. Available at \url{https://github.com/discovery-unicamp/memory-aware-chunking/blob/main/experiments/02-predicting-memory-consumption-from-input-shapes/scripts/experiment.sh}. Accessed: 2025-04-29.} automates the entire pipeline.
Users must install Docker, clone the repository, and execute \texttt{experiment.sh} to reproduce every data processing and analysis stage.
The \texttt{notebooks/} directory~\footnote{Notebooks Folder. Available at \url{https://github.com/discovery-unicamp/memory-aware-chunking/blob/main/experiments/02-predicting-memory-consumption-from-input-shapes/notebooks}. Accessed: 2025-04-29.} includes Jupyter notebooks for data exploration, model training, hyperparameter tuning, and error diagnostics.

...
\begin{itemize}
    ...
    Envelope and Gaussian Filter, by contrast, showed comparatively gentler linear relationships, suggesting that their computational kernels \EBC{rely less on large intermediate buffers}{Se a curva é mais inclinada, então ele deve estar dependendo mais de buffers intermediários, não?}.
    ...
    \ac{GST3D} occasionally benefited from additional parameters (\eg, diagonal length), but not enough to outweigh the straightforward predictive power of volume.
    \EB{E se o volume não for usado? Ou se apenas as features originais forem usadas?}
...
\begin{enumerate}
    \item \emph{Volume is the primary explanatory feature} for predicting peak memory usage \EBADD{for the evaluated operators}, with additional variables offering only minor gains.
...
With these findings, we have a clearer framework for implementing memory-aware chunking strategies \EBADD{for applications that rely on these operators}: one can rely on volume-based models (potentially augmented by minimal shape features) and still maintain accurate peak \ac{RAM} consumption estimates, even with relatively small training sets.

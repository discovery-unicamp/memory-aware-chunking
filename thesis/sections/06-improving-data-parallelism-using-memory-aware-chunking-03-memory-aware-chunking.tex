...
This approach leverages a trained regression model, detailed in \textbf{Chapter~\ref{ch:predicting-memory-consumption-from-input-shapes}}, enabling accurate predictions of memory requirements, thereby facilitating informed decisions about optimal chunk sizes.
...
\begin{enumerate}
    ...
    \item \textbf{Chunk Dimension Selection:}
    The algorithm translates the predicted total memory consumption into a per-voxel memory cost.
    Using this value, it calculates an initial chunk dimension $c$, adhering to the constraint:
    \begin{equation}
...
The Python implementation of this logic resides in the \texttt{\_find\_memaware\_chunk\_size} function within the script \texttt{collect\_profile.py}.
...
These configurations, represented by environment variables such as \texttt{MEMORY\_LIMIT\_GB} (\eg, 16 GB) and \texttt{SAFETY\_FACTOR} (\eg, 0.8), allow users to control memory utilization relative to their available computational resources dynamically.
...
\begin{itemize}
    ...
    \item \textbf{Predictable and Efficient Scaling:} Memory-aware chunking systematically selects chunk dimensions that distribute workloads evenly across parallel workers, enhancing computational efficiency and resource utilization without being restricted by arbitrary file-block boundaries or heuristic thresholds.

    \item \textbf{Minimized Manual Hyper-Parameter Tuning:}
    By automating the chunk size determination process through predictive modeling, the proposed approach significantly reduces the need for manual tuning.
    This automation minimizes computational overhead associated with iterative, trial-and-error methods, saving considerable time and computational resources.

...
\begin{enumerate}
    ...
    \item \textbf{Explicit Resource Management:} Containers allow defined memory constraints using flags such as \texttt{–memory} or \ac{cgroup} configurations, useful for validating memory usage limits (detailed in Section~\ref{subsec:mmc-bestpractices-validation}).
\end{enumerate}
...
Tools such as \texttt{psutil}~\cite{psutil}, Docker statistics, and Linux’s \texttt{/proc} filesystem measure system-wide metrics including \ac{RSS}, \ac{VMS}, and shared memory.\EB{Talvez fosse importante ter uma seção no capítulo 2 explicando estes termos/conceitos -- RSS, VMS, \etc. O que acha?}
...
Selecting appropriate measurement frequency and granularity is crucial to accurately capture memory usage without excessive overhead:

\begin{resumo}
    ...
    Por meio de profiling de memória utilizando a biblioteca original (TraceQ), o uso de memória foi precisamente capturado em ambientes controlados e conteinerizados.
    ...
    Estudos de seleção de features e redução de dados demonstraram que o volume total dos dados fornece capacidade preditiva suficiente, simplificando o processo.
    ...
    Embora ocasionalmente introduza leve aumento no tempo de execução devido ao particionamento conservador, a substancial melhora em estabilidade operacional justifica esse trade-off, especialmente em cenários críticos onde robustez supera ganhos marginais de desempenho.

    Em suma, esta dissertação introduz um método prático e automatizado que garante robustez em workflows Dask por meio do dimensionamento inteligente de partições.
...
\begin{abstract}
    ...
    This thesis presents an automated memory-aware chunking methodology that significantly enhances the reliability of Dask-based applications by accurately predicting memory consumption from input data dimensions.
    Initially, a systematic evaluation was conducted to identify the key factors impacting memory usage in data-intensive workloads, specifically targeting seismic data processing operators (Envelope, \ac{GST3D}, and \ac{3D} Gaussian Filter).
    ...
    Extensive feature selection and data reduction studies validated that input volume alone provided sufficient predictive power, simplifying the complexity of the prediction process.
    ...
    Rather than optimizing purely for speed, the algorithm prioritizes robustness, proactively adjusting chunk dimensions to prevent memory overflow.
    Empirical evaluation showed that the proposed approach eliminated \ac{OOM} errors entirely, providing consistent reliability even when handling very large seismic datasets.

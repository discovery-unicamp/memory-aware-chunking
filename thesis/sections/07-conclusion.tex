...
\begin{enumerate}
    ...
    Rather than relying on a single utility such as \texttt{top} or \texttt{tracemalloc}, both internal (Python-level) and external (\ac{OS}-level) metrics ensure transient allocations are captured while instrumentation overhead remains limited.
...
\begin{itemize}
    ...
    Among shape-derived features (\eg, surface area, aspect ratios, diagonal lengths), the volume term explains the majority of the variance (often exceeding 99\%).
...
Conventional chunking heuristics (\eg, Dask’s “auto-chunking”) often struggle with higher-dimensional \EBADD{data} or \EBC{HPC contexts}{distributed HPC contexts?}.
...
\begin{itemize}
    ...
    The tested operators (Envelope, \ac{GST3D}, Gaussian Filter) exhibit linear scaling, but other workloads such as \EBC{deep-learning pipelines}{acho que os pipelines de deep-learning serão bem comportados e previsíveis também} and other operators might behave more unpredictably.
...
They directly apply to industrial and academic seismic processing pipelines and \EBADD{can improve} a wide range of large-scale tensor-based computations, such as geospatial imaging, bioinformatics, or climate modeling.

...
Python's internal memory model, reference counting, cyclic garbage collection, and dynamic allocation make it challenging to capture stable numbers for peak or average memory usage.
Container-based abstractions (\eg, Docker or other \ac{cgroup}-based solutions) introduce further variability due to filesystem caches, shared kernel resources, and copy-on-write behaviors.

Several practices address these complexities.
\begin{enumerate}
    \item Clean-Slate Isolation.
    Lightweight containers or fresh Python subprocesses for each measurement iteration prevent remnant state, memory fragmentation, or objects left over from prior runs.
    \item Cross-Validation with Multiple Tools.
    Rather than relying on a single utility such as \texttt{top} or \texttt{tracemalloc}, the experiment captures both internal (Python-level) and external (\ac{OS}-level) metrics to ensure transient allocations are measured while limiting instrumentation overhead.
...
The specialized open-source library \traceq streamlines these measurements.
Empirical tests show that \traceq tracks peak memory usage within 2--3\% of \ac{OS}-level benchmarks with minimal impact on performance.


\section{Predicting Memory Consumption from Input Shape}
...
\begin{itemize}
    ...
    Among shape-derived features (\eg, surface area, aspect ratios, diagonal lengths), the volume term explains most of the variance (often exceeding 99\%).
    ...
    Complex regressors (\eg, gradient boosting or neural networks) provide only minor gains compared to simpler linear or polynomial regressions, making it practical for \ac{HPC} teams to use faster-to-train models without losing accuracy.
\end{itemize}
...
Such limited sampling reduces overhead when profiling large shape grids.


\section{Memory-Aware Chunking for Data Parallelism}
\label{sec:conc-chunking}

The third phase integrates predictive memory models into a memory-aware chunking scheme for data-parallel tasks like those managed by Dask.
Conventional chunking heuristics (\eg, Dask's "auto-chunking") often struggle with higher-dimensional data or distributed \ac{HPC} contexts.
...
\begin{itemize}
    ...
    The memory-aware method avoids out-of-memory incidents even at higher worker counts across hundreds of runs on medium to large \ac{3D} volumes.
    ...
    Memory-aware chunking on small data can be slower by 5--15\% compared to automatic or evenly-split chunking.
...
This trade-off becomes highly appealing in HPC operations where out-of-memory errors invalidate entire runs.
...
\begin{enumerate}
    ...
    A tested strategy involving containerized isolation, multi-tool cross-validation, carefully chosen sampling frequencies, and the \traceq library.
...
\begin{itemize}
    ...
    The tested operators (Envelope, \ac{GST3D}, Gaussian Filter) exhibit linear scaling, but other workloads, such as other operators, might behave more unpredictably.
    Applying memory-aware chunking to a broader set of operators demands re-profiling, retraining, or refining the predictive model.
    \item \ac{GPU} Memory.
    ...
    Extensions of \traceq or similar tools could track \ac{GPU} device allocations in real time.
...
The precise measurement, lightweight predictive models, and robust data-parallel frameworks bridge the gap between Python memory introspection and \ac{HPC} resource requirements.
...
They directly apply to industrial and academic seismic processing pipelines and can improve large-scale tensor-based computations, such as geospatial imaging, bioinformatics, or climate modeling.

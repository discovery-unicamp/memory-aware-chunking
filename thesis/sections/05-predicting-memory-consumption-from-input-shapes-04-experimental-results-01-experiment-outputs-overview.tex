...
Our primary goal was to build regression models that predict each operator's peak memory usage as a function of shape parameters (inlines, xlines, and samples), enabling more optimized \ac{HPC} job submissions.

\vspace{1em}
\noindent
\textbf{Data Artifacts.}
The final stage of the pipeline generated six main \ac{CSV} outputs:
\begin{enumerate}
    \item \emph{profile\_history.csv.xz}: Stores time-series \ac{RSS} measurements and timestamps for each operator run. This file is compressed with \emph{xz}~\cite{xz} to mitigate large file sizes caused by high-resolution time-series logging.
    ...
    \item \emph{data\_reduction.csv}: Compares how models behave when trained on progressively smaller subsets of the whole dataset, providing insight into data requirements for stable predictions.
    \item \emph{model\_tuning.csv}: Records the best hyperparameters discovered for each model-operator combination (via k-fold cross-validation and \texttt{Optuna}), including any search-space parameters used to optimize the multi-metric objective.
\end{enumerate}
...
We could characterize memory usage trends across diverse problem scales by spanning small and large volumes.

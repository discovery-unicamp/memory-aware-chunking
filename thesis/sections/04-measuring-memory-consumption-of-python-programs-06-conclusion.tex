...
The \texttt{traceq} \EBC{library}{library or tool?} consolidates these measurement techniques and manages high‑frequency sampling with minimal overhead.
...
\EBRP{Future}{The following} chapters will build on these strategies to predict memory consumption from input shapes and improve data parallelism with memory‑aware chunking, using the same measurement methodologies to ensure dependable results in large‑scale or performance‑sensitive environments.

\EB{Daniel, penso que a ferramenta (toolkit?) \traceq é uma das contribuições do seu trabalho e deveria ser listada na lista de contribuições na introdução da dissertação.}
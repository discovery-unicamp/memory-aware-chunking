...
This section explains the complete methodology for gathering memory-consumption data, generating shape-based features, training, and evaluating the predictive models.
The overall pipeline includes four core phases.
(i) Data Generation,
(ii) Memory Profiling,
(iii) Result Collection,
and (iv) Analysis.
These phases ensure consistent, reproducible, and high-quality measurements, which support \ac{ML} models that learn memory usage from shape attributes.
...
These synthetic datasets mimic actual seismic data and span diverse volumes, revealing how memory grows with increasing dimension sizes.

\vspace{1em}
\noindent
\textbf{Why Synthetic Data Does Not Add Bias.}
Some concerns may arise about whether synthetic datasets reflect actual operational data.
The methodology includes a validation step using the publicly available F3 real seismic dataset to address these concerns.
The results show that synthetic volumes' memory-consumption patterns and shape-to-usage relationships closely match those observed in real seismic data.
This consistency confirms that the measured trends remain valid in both controlled (synthetic) and real-world conditions, thereby mitigating any risk of bias introduced by synthetic data.
...
The \traceq library records \ac{RSS} over time, as discussed in Chapter~\ref{ch:measuring-memory-consumption}.
...
moreover, (v) finalize executing another Python script for exploratory data analysis.
Each run deposits output \ac{CSV} files and profiles in designated \texttt{OUTPUT\_DIR} subdirectories.

\vspace{1em}
\begin{lstlisting}[
    style=bashstyle,
    caption={Excerpts from \texttt{experiment.sh} that orchestrate Docker-based runs. Variables like \texttt{DATASET\_FINAL\_SIZE} and \texttt{DATASET\_STEP\_SIZE} define shape ranges for dataset generation.},
    label={lst:exp_sh}
]
#!/usr/bin/env sh
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        \hline
        \texttt{DATASET\_FINAL\_SIZE} & Final dimension size (\eg, 800)                                                            \\
        \texttt{DATASET\_STEP\_SIZE}  & Step size between dimension increments (\eg, 100)                                          \\
        \texttt{EXPERIMENT\_N\_RUNS}  & Number of repeated runs per shape (\eg, 5)                                                 \\
        \texttt{CPUSET\_CPUS}         & \ac{CPU} core pinning for Docker containers (\eg, 0)                                       \\
        \texttt{OUTPUT\_DIR}          & Directory for output profiles, logs, and \texttt{\ac{CSV}} files                           \\
        \texttt{ALGORITHM}            & Selected operator (\eg, \texttt{envelope}, \texttt{\ac{GST3D}}, \texttt{gaussian\_filter}) \\
        \hline
    \end{tabular}
\end{table}
...
This phase consists of evaluating the \texttt{.prof} files produced by \traceq, extracting peak memory usage, and associating it with the input shape.
...
These features capture various ways in which shape correlates with peak memory usage.
Listing~\ref{lst:results_py} shows how this phase generates and transforms these features.

\begin{lstlisting}[style=pythonstyle,
    caption={Feature extraction excerpt from \texttt{collect\_results.py}. Additional derived metrics capture polynomial or ratio-based growth.},
...
Phase 4 trains multiple regression models using inlines, xlines, samples, and derived features as memory consumption predictors.
Peak memory usage serves as the target variable.
An 80/20 training/test split ensures fair comparisons.
Each model's hyperparameters are selected using a k-fold cross-validation~\cite{kohavi1995study} procedure on the training portion.
This step provides a validation mechanism that avoids leaking information from the test set.
After selecting hyperparameters, the script retrains the model on the entire training set and evaluates the final performance on the held-out test set.
Four standard regression metrics—\ac{RMSE}, \ac{MAE}, \(R^2\), and a custom "accuracy" measure—assess predictive quality.
...
During hyperparameter tuning, these metrics form a single optimization target:
\[
    \texttt{score} \;=\;
    ...
    w_{R^2}\,\cdot\,R^2,
\]
where \(w_\mathrm{acc}, w_\mathrm{RMSE}, w_\mathrm{MAE},\) and \(w_{R^2}\) indicate how strongly the hyperparameter optimization process rewards or penalizes each metric.
Maximizing this composite \(\texttt{score}\) during tuning balances the risk of underestimating memory (which can trigger job failures) against the cost of over-provisioning (which wastes resources).

The \texttt{Optuna} framework~\cite{akiba2019optuna} orchestrates hyperparameter tuning.
Each trial within the study trains the model using a k-fold split on the training data and measures a combined score.
By maximizing that score, the method identifies the hyperparameters that best balance accuracy, \ac{RMSE}, \ac{MAE}, and \(R^2\).
...
\begin{itemize}
    ...
    Polynomial regression extends linear models with higher-order terms to account for polynomial nonlinearities.
    ...
    Hyperparameter tuning (\eg, tree depth or learning rate) is essential to avoid overfitting~\cite{bergstra2012}.

    \item \textbf{Neural Networks (\ac{MLP})}~\cite{rumelhart1986learning}\\
    \ac{MLP} architectures can approximate intricate functions with sufficient capacity.
...
The final Python script then visualizes memory distributions, residual plots, feature evaluations, and data-reduction experiments.
This exploration reveals each model's generalization capacity, indicates which features add minimal value, and evaluates the number of training examples the predictive framework requires.
Partial dependence plots and tree-based model interpretations expose how inlines, xlines, and samples influence memory usage most.
...
\begin{itemize}
    ...
    Multiple runs per dataset shape further stabilizes performance measurements by smoothing out random fluctuations, often described as stochastic variance.
...
\begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tikzpicture}[
            ...
            \node[phase, right=3cm of phase2] (phase3) {Phase 3:\\Aggregation and Feature\\Construction};
            ...
            \node[phase, right=3cm of phase3] (phase4) {Phase 4:\\Model Training and Evaluation};

            % (A) "Hyperparam Tuning (K-fold)"
            \node[box, below=of phase4, text width=3.8cm] (p4a) {
                \textbf{Process}\\
            ...
            % (B) "Train & Predict"
            \node[box, below=of p4a, text width=3.8cm] (p4b) {
                \textbf{Process}\\
                Final training on the entire train set\\
                + Test set evaluation
            };

            \node[box, below=of p4b] (p4output) {
                \textbf{Output}\\
                Memory usage predictions
            };

            \draw[arrow] (p3output.east) -- ++(0.7,0) |- (p4a.west);
            \draw[arrow] (p4a) -- (p4b);
            \draw[arrow] (p4b) -- (p4output);

        \end{tikzpicture}
    }
    \caption{Schematic of the memory prediction experiment.
    It includes synthetic data generation, containerized profiling, feature extraction, and regression model training.

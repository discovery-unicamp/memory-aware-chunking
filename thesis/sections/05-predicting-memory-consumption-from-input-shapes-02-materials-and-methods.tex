...
This section explains the complete methodology for gathering memory-consumption data, generating shape-based features, \EBRP{and training}{training, and evaluation the} predictive models.
...
\EB{Talvez seja importante argumentar que o uso de dados sintéticos não causa viés na análise por que o perfil de computação e consumo de memória dos operadores usados no processamento do dado não dependem dos valores das amostras em si, apenas do shape do dado.}

\vspace{1em}
\noindent
\textbf{Phase 2: Memory Profiling.}
A shell script schedules container-based jobs to run \EBC{seismic operators}{Não lembro se você definiu o que são seismic operators. Se não definiu, talvez vale adicionar uma breve seção explicando as aplicações e/ou operadores - estas aplicações/operadores fazem parte dos materiais usados no experimento.} on each generated dataset.
...
This phase consists in evaluating the \texttt{.prof} files \EBRP{output}{produced} by \texttt{traceq}, extracting peak memory usage, and associating it with the input shape.
The script identifies \EBC{inlines, xlines,}{Estes termos foram definidos?} and samples from filenames, notes the operator, retains the maximum time-series \ac{RSS} value, and compiles a consolidated dataframe.
...
Phase 4 trains multiple regression models using inlines, xlines, samples, and derived features as \EBADD{memory consumption} predictors.
...
\EB{Daniel, a busca de hiperparâmetros deve ser feita com um conjunto de treino e validação, sem usar o conjunto de teste - que deve ser reservado para a avaliação final. É importante verificar se foi assim mesmo que você executou o exepeimento e explicar esta metodologia aqui.}
...
\begin{itemize}
    ...
    Polynomial regression extends linear models with higher-order terms to account for \EBRP{mild}{polynomial} nonlinearities.
...
\begin{figure}[htbp]
    ...
    \caption{Schematic of the memory prediction \EBRPD{pipeline}{experiment}.
    It includes synthetic data generation, containerized profiling, feature extraction, and regression model training.
    \EB{Feature Construction $\rightarrow$ Feature Extraction?}

...
Standard \ac{HPC} job schedulers, such as Slurm~\cite{yoo2003slurm} or \ac{PBS}~\cite{henderson1995pbs}, enforce fixed memory reservations at submission time.
...
For clarity, the term "tensor-based" (or equivalently, "tensor-centric") describes computations centered on these large data structures, sometimes also called "array-based" workloads, which exhibit strong correlations between memory usage and input shape.
Numerous scientific and data-centric applications center on these large multidimensional arrays.
Examples include seismic wave simulations on \ac{3D} grids~\cite{okamoto2010}, high-resolution image analysis in computer vision~\cite{yuyang2024}, and large-scale numerical linear algebra routines in scientific computing.
...
For example, doubling the resolution of an input image in each dimension roughly quadruples its memory footprint~\cite{stackoverflow_memory_inv}. In contrast, increasing either input size or batch size in deep neural networks similarly amplifies memory usage due to larger activation maps and additional computation~\cite{dell_3dunet_memory}.
...
A predictive model that captures how memory scales with shape parameters can offer robust estimates for new input configurations.

Several research directions have explored memory usage estimation and prediction in related fields.
In the \ac{HPC} domain, historical job data from systems like Slurm often enables \ac{ML} models to predict resource consumption.
...
In high-level numerical computing, analytical memory modeling for array operations provides another possibility~\cite{cornell_memory_workshop}.
Matrix multiplications, convolutions, and other tensor-based methods can be studied to deduce peak memory usage without complete execution.
...
Focusing on a reliable upper-bound estimate allows users to reserve memory more confidently.

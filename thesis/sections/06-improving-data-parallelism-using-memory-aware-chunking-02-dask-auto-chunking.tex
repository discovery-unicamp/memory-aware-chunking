...
At the heart of Dask's data partitioning strategy is the \emph{auto-chunking} feature, which automatically determines suitable chunk sizes based on heuristic-driven approaches.
...
This heuristic approach frequently yields effective and efficient chunk sizes in simpler, one-dimensional scenarios.
However, the heuristic strategy often struggles with higher-dimensional datasets, complex computations such as seismic analysis, and tensor-based operations.

The simplistic heuristic frequently results in uneven or tiny chunks in multi-dimensional contexts.
...
Furthermore, Dask's default chunking heuristic does not explicitly consider algorithm-specific complexities or intermediate data expansions typical in sophisticated computational workflows.
...
For instance, processing a moderate-sized volume like $600\times600\times600$ could inadvertently result in numerous inadequately small chunks or huge chunks, potentially causing \ac{OOM} errors in memory-constrained environments.
...
Dask's auto-chunking mechanism cannot incorporate task-specific and operator-specific information, leading to suboptimal performance in demanding scenarios typical of seismic and high-dimensional imaging applications.
Addressing this shortcoming, the subsequent section introduces \emph{memory-aware chunking}. This predictive approach leverages an algorithm-specific memory usage model to intelligently guide chunk size selection, enhancing memory efficiency and computational performance.
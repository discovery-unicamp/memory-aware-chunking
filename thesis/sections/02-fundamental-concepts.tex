...
\end{itemize}
In Python-based \ac{HPC} environments, libraries like Pandas or Dask handle transformation steps at scale, enabling distributed processing of large datasets~\cite{Rocklin2015}.
...
\begin{itemize}
    ...
    \item \emph{Distributed Scheduler:} Typically used in an \ac{HPC} or multi-machine context. A central scheduler process coordinates a cluster of workers executing subsets of the task graph~\cite{Rocklin2015}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Memory Efficiency:} Chunks that are too significant increase the likelihood of \ac{OOM} failures on a worker, as each chunk requires processing in \ac{RAM}.
...
\paragraph{Takeaways.}
By building and executing tasks from a \ac{DAG}, Dask enables parallelism across many computational resources, making it well-suited for data-intensive tasks in Python~\cite{Rocklin2015}.
...
Frameworks like Dask enable splitting arrays or dataframes into smaller \emph{chunks} across multiple workers, facilitating concurrent execution~\cite{Rocklin2015,Dean2004}.

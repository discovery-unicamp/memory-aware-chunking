...
Aside from reference counting, Python also uses a cyclic garbage collector to handle objects that reference each other (so-called "reference cycles"), simple reference counting alone cannot reclaim these objects~\cite{Nanjekye2023}.
...
Metrics may also undercount peaks if certain memory pages remain cached without appearing as "used" in standard \ac{OS} metrics~\cite{Nanjekye2023}.
These conditions complicate interpreting a single memory usage metric.
...
A small "balloon" driver within each guest (or container runtime) inflates to claim unused memory pages, effectively returning them to the host or other guests under memory pressure.
...
Too small chunks might hamper parallel efficiency, while oversized chunks can exceed memory capacity and trigger \ac{OOM} errors.

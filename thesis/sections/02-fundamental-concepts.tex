...
Data-parallel frameworks such as Dask further \EBC{complicate or enhance}{qual é o ponto aqui? Argumentar que ele ajuda ou que ele atrapalha?} performance by partitioning data into chunks, where naive chunking strategies can ignore memory-intensive stages.
...
Memory usage observed just before a garbage-collection cycle might look higher than at other times, while reclaimed memory might still remain allocated within Python’s private arenas. \EB{O que é uma arena?}
...
Containerization packages software dependencies and environment configuration into isolated bundles that share the host’s kernel while separating file systems, namespaces, and \EBC{cgroups}{Talvez cgroups não seja um termo tão conhecido - que tal usar ``and other computing resources such''? }.
...
\EB{Lembre-se de definir as siglas e acrônimos antes do primeiro uso - BTW, não conhecia o termo DIND.}
...
\EB{Senti falta de referências para o projeto/site/artigo que apresenta o DIND.}
...
Frameworks like Dask allow arrays or dataframes\ADDREF to be split into smaller \emph{chunks} across multiple workers, enabling concurrent execution.
...
\EB{Seria legal ter uma imagem aqui ilustrando o processamento em chunks. De preferência uma imagem que ajude a visualizar os problemas de selecionar chunks muito pequenos (excesso de tarefas para escalonar), e chunks muito grandes. Se a imagem na introdução já deixar estes conceitos claros, então não vejo necessidade de adicionar outra aqui.}
...
Insufficiently sized chunks might hamper parallel efficiency, while oversized chunks may exceed memory capacity \EBADD{and lead to \ac{OOM}}.


\section{Predictive Modeling in HPC Resource Management}
\label{sec:fundamentals-predictive-modeling}

Resource managers \EBADD{can} benefit significantly from predictive models that forecast memory usage or runtime based on job features.
...
Historical-based solutions gather logs of prior runs and fit regressions or machine-learning models to estimate resources for similar future jobs. \EB{Adicione referências para trabalhos que usam esta abordagem}
...
\EB{Se houver, adicione referências para trabalhos que usam esta outra abordagem}

\subsection{Shape-Based Memory Prediction}
\label{subsec:fundamentals-shape-prediction}

\EBC{Tensor-centric}{Não estou certo ainda, mas talvez seja melhor evitar o uso do termo tensor} tasks often revolve around multi-dimensional data, where memory usage correlates closely with array volume or dimension sizes.
...
Empirical studies repeatedly show near-linear relationships between array size and peak memory for filters, transforms, or deep learning layers. \EB{Adicione referências para estes estudos.}

\subsection{Chunking Decisions via Modeling}
\label{subsec:fundamentals-chunking-decisions}

Predictive models \EBC{help}{help or can help?} \ac{HPC} frameworks decide chunk sizes based on the memory footprint expected per chunk, factoring in overhead from the operators in question.
